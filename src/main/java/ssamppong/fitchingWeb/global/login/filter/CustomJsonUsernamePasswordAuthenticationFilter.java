package ssamppong.fitchingWeb.global.login.filter;
...
public class CustomJsonUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException, ServletException {
        if(request.getContentType() == null || !request.getContentType().equals(CONTENT_TYPE)){
            throw new AuthenticationServiceException("Authentication Conetent-Type not supported: " + request.getContentType());
        }
        ...
        log.info("일반 회원 로그인 요청 : {}", email); // 비밀번호는 보안 우려로 넣지 않음
